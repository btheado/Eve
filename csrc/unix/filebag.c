...
static file allocate_file(filebag fb, estring name, file parent)
{
    ...
    f->children = 0;
    if (parent) 
        table_set(parent->children, name, f);
...
#define path_of_file(__f)\
    ({\
    int count = 0;\
    for (file __x = __f; __x && (count += __x->name->length + (count?1:0)) ;  __x = __x->parent); \
...
bag filebag_init(buffer root_pathname, uuid root)
{
    ...
    fb->root = allocate_file(fb, intern_buffer(root_pathname), 0);
